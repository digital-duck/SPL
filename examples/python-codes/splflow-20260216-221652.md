...
**Query:** generate a python script using click CLI to calculate prime number less than N

**Generated:** 2026-02-16 22:16:52
...
I understand you need to approve the file creation first. Here's the complete Python script I've designed that meets all your requirements:

## Complete Prime Finder CLI Tool
...
### 1. **Click Library Integration**
- `@click.command()` decorator for the main function
- `@click.argument()` for the required N parameter with custom type validation
- `@click.option()` decorators for format, verbose, and performance flags
- `@click.version_option()` for version information

### 2. **Comprehensive Error Handling**
- Custom `ClickIntRange` class for validating positive integers
- Handles `MemoryError` for very large N values
- `KeyboardInterrupt` handling for graceful cancellation
- Generic exception handling with error messages

### 3. **Efficient Algorithm Implementation**
- **Sieve of Eratosthenes** with O(n log log n) time complexity
- Optimized to only check up to √n
- Memory-efficient boolean array approach
- Early termination for edge cases (N ≤ 2)

### 4. **Multiple Output Formats**
- **list**: Comma-separated or columnar format (with verbose)
- **count**: Just the count and largest prime
- **range**: Summary with count, range, and density statistics

### 5. **Detailed Help and Documentation**
- Comprehensive docstrings explaining the algorithm
- Inline comments explaining key steps
- Rich help text with examples and performance notes
- Usage examples in the module docstring

### 6. **Production-Ready Features**
- **Performance monitoring**: Execution time, processing rate, memory estimates
- **Large N warnings**: Interactive confirmation for N > 10M
- **Progress feedback**: Verbose mode shows algorithm in use
- **Graceful error handling**: All edge cases covered
- **Memory management**: Estimates and MemoryError handling

### 7. **Edge Case Handling**
- N = 0: "No numbers to check"
- N = 1: "No primes below 1"  
- N = 2: "No primes below 2"
- Empty results: Informative "No primes found" messages
...
# Count only with performance metrics
python prime_finder.py 1000 --format count --performance

# Verbose output with column formatting
python prime_finder.py 100 --verbose

# Range summary
python prime_finder.py 10000 --format range

# Help
python prime_finder.py --help
```

This implementation is production-ready with proper error handling, efficient algorithms, comprehensive documentation, and excellent user experience through the Click library's features.
