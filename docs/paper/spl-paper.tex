...
\begin{enumerate}[nosep]
  ...
  \item \textbf{RAG}: FAISS/ChromaDB vector search (milliseconds, minimal cost)
...
\begin{figure}[h]
\centering
\includegraphics[width=0.62\textwidth]{figures/arch_diagram.pdf}
\caption{SPL engine pipeline architecture. The compiler stages (blue input,
green pipeline) transform SPL source into an execution plan consumed by the
executor (yellow), which dispatches to three back-end subsystems (pink):
LLM adapters (OpenRouter / Claude CLI), a SQLite memory store, and a vector
RAG store supporting both FAISS and ChromaDB (purple).}
\label{fig:arch}
\end{figure}
...
\begin{description}[nosep]
  ...
  \item[Vector Store] (\texttt{.spl/vectors.*}): Provides semantic similarity search for \texttt{rag.query()}. The engine supports two backends: \textbf{FAISS}~\citep{johnson2019billion} for file-based, zero-configuration deployment, and \textbf{ChromaDB}~\citep{chromadb2023} for richer metadata filtering and a managed-server mode. Both expose an identical Python interface, making the choice transparent to SPL queries. Document metadata is stored in a companion SQLite database (\texttt{.spl/vectors\_meta.db}).
\end{description}
...
\begin{enumerate}
  ...
  interfaces in traditional databases~\citep{androutsopoulos1995natural} and shares
  conceptual roots with the NL-to-SQL research lineage pioneered by the Yale
  team~\citep{yu2018spider,yu2019sparc}.
  ...
  Crucially, MoM routing \emph{bypasses the need for explicit agentic workflows}
  altogether.  Rather than orchestrating a graph of agents with message-passing,
  retries, and state management, the developer writes a single declarative SPL
  script; the router silently assigns each \texttt{PROMPT} clause to its optimal
  specialist model at execution time.  The result is an \emph{AI Symphony}:
  a multi-CTE query in which each voice---CJK text rendered by a language
  specialist, code generated by a coding model, final synthesis performed by a
  reasoning model---plays its part on cue, coordinated not by imperative
  orchestration code but by the declarative structure of the query itself.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Integrated context management}: Built-in RAG (FAISS/ChromaDB) and memory (SQLite) eliminate the need for external context infrastructure.
\end{enumerate}
...
As the author's own transition from data engineering to AI engineering illustrates---first
through building a RAG-powered analytics application~\citep{gong2024datacopilot} inspired
by years of Oracle and SQL practice, and then through SPL itself---the conceptual leap
from \texttt{SELECT} over relational tables to \texttt{PROMPT}/\texttt{SELECT}/\texttt{GENERATE}
...
The NL-to-SQL community represents an especially natural early adopter group.
Researchers and practitioners who work on translating natural language to SQL
\citep{yu2018spider,yu2019sparc} are already fluent in the core paradigm---mapping
free-form intent to a structured query language---and the extension from
NL$\to$SQL to NL$\to$SPL is the smallest possible conceptual leap.
Where NL-to-SQL systems target a \emph{static} knowledge base (relational tables),
Text2SPL targets a \emph{generative} one (LLM weights), making SPL the natural
next frontier for this research community.
...
The author also thanks the open-source communities behind FAISS, ChromaDB, tiktoken, and the
