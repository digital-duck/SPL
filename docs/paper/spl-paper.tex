...
% === Title ===
\title{%
  \textbf{Structured Prompt Language: Declarative Context Management\\
  for Large Language Models}
}

\author{
  Wen Gong\thanks{Corresponding author. Contact: \url{https://github.com/digital-duck}}
  \and
  Claude Opus 4.6\thanks{AI collaborator (Anthropic). Implementation, synthesis, and paper co-authorship.}
}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Reference implementation}: We provide a complete, pip-installable Python package (\texttt{spl-lang}) with a hand-written recursive descent parser, optimizer, and provider-agnostic execution engine.
...
\toprule
\textbf{Feature} & \textbf{SPL} & \textbf{Prompty} & \textbf{DSPy} & \textbf{LMQL} & \textbf{LangChain} \\
\midrule
...
Provider-agnostic & \checkmark & Partial\textsuperscript{b} & \checkmark & \checkmark & \checkmark \\
...
\subsection{Qualitative Analysis: Developer Experience}

To illustrate SPL's developer experience advantages, we compare implementing a RAG-augmented question-answering system using imperative Python versus SPL.

\subsubsection{Imperative Approach (Python + LangChain)}

\begin{lstlisting}[style=python, caption={Imperative RAG implementation (simplified)}]
import tiktoken
...
def build_prompt(question, max_tokens=8000):
    ...
    for doc in docs:
        ...
        if doc_tokens <= remaining:
            ...
            # Crude truncation
            doc_text += doc.page_content[:remaining * 4]
            break
...
\subsubsection{SPL Approach}

\begin{lstlisting}[language=SPL, caption={Equivalent SPL query}]
PROMPT answer_question
WITH BUDGET 8000 tokens
USING MODEL claude-sonnet-4-5
...
SELECT
    system_role("You are a knowledgeable assistant"),
    context.question AS question LIMIT 200 tokens,
    rag.query("relevant docs", top_k=5) AS docs
        LIMIT 3000 tokens,
    memory.get("history") AS history LIMIT 500 tokens
...
\begin{table}[h]
\centering
\caption{Imperative vs.\ SPL: developer experience comparison.}
\label{tab:dx-comparison}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Metric} & \textbf{Imperative} & \textbf{SPL} \\
\midrule
Lines of code & 30+ & 12 \\
Token budget visibility & Hidden in variables & Declared in syntax \\
Execution plan & Not available & \texttt{EXPLAIN} \\
RAG integration & External library & Built-in (\texttt{rag.query}) \\
Memory integration & External library & Built-in (\texttt{memory.get}) \\
Model portability & Requires code changes & Change \texttt{USING MODEL} \\
Testability (static) & Requires execution & \texttt{spl validate} \\
\bottomrule
\end{tabular}
\end{table}
...
The reference implementation passes a comprehensive test suite covering:

\begin{itemize}[nosep]
  \item \textbf{Lexer}: 14 tests covering tokenization of keywords, strings, numbers, operators, comments, dot notation, and error cases (unterminated strings).
...
All 40 tests pass, validating the correctness of the lexer, parser, analyzer, and optimizer pipeline.
...
\subsection{Toward an SPL Standard}

SQL's greatest impact came not from any single implementation but from standardization (ANSI SQL-86, SQL-92, SQL:1999, etc.). We envision a similar trajectory for SPL:

\begin{enumerate}[nosep]
  \item \textbf{SPL-1.0} (this paper): Core language with PROMPT, SELECT, GENERATE, CTEs, functions, EXPLAIN.
  \item \textbf{SPL-2.0}: Transactions (multi-step prompting with rollback), materialized prompts (cached computation), streaming generation.
  \item \textbf{SPL-3.0}: Multi-modal context (images, audio as context sources), cross-model orchestration, federated context (querying multiple LLMs in a single SPL statement).
\end{enumerate}
...
This project itself was developed through human-AI collaboration: the first author provided the vision, domain expertise (20+ years of SQL/Oracle experience), and architectural decisions, while the AI collaborator provided implementation, code generation, and synthesis. The complete co-creation log documenting this process is available as supplementary material. We believe this collaborative development methodology---where human domain expertise guides AI implementation capability---represents a productive model for future software development.
...
Just as SQL (1970) brought order to the chaos of database access, we believe SPL (2026) can bring order to the chaos of LLM prompt engineering. The reference implementation is available as an open-source Python package (\texttt{pip install spl-lang}) at \url{https://github.com/digital-duck/SPL}.
