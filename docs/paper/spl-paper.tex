...
\begin{description}[nosep]
  ...
  \item[Vector Store] (\texttt{.spl/vectors.*}): Provides semantic similarity search for \texttt{rag.query()}. The engine supports two backends: \textbf{FAISS}~\citep{johnson2019billion} for file-based, zero-configuration deployment, and \textbf{ChromaDB} for richer metadata filtering and a managed-server mode. Both expose an identical Python interface, making the choice transparent to SPL queries. Document metadata is stored in a companion SQLite database (\texttt{.spl/vectors\_meta.db}).
\end{description}
